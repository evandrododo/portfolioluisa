...
function useScrollSnap({ ref = null, duration = 100, delay = 50 }) {
    ...
    const tickAnimation = useCallback((value) => {
        if(!targetScrollOffsetRef || !currentScrollOffsetRef) return;
        const scrollTopDelta = Number(targetScrollOffsetRef.current) - Number(currentScrollOffsetRef.current);
        const scrollTop = Number(currentScrollOffsetRef.current) + (scrollTopDelta * value / 10000);
